            ...
            <div key={transaction.id} className="p-4 hover:bg-gray-50 transition-colors">
              <div className="flex items-center justify-between">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {transaction.parent_transaction && transaction.parent_transaction.length > 0 && (
                      ...
                      <div className="flex items-start space-x-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs text-blue-700 font-medium mb-1">
                            References {transaction.parent_transaction.length} transaction{transaction.parent_transaction.length > 1 ? 's' : ''}:
                          </p>
                          <div className="space-y-1">
                            {transaction.parent_transaction.map((parentTx, index) => (
                              <div key={parentTx.id} className="text-xs text-blue-600">
                                <span className="font-medium">
                                  {parentTx.person?.name || 'Unknown'} • {formatAmount(parentTx.amount)}
                                  {parentTx.transaction_type === 'credit' ? ' (They owe)' : ' (I owe)'}
                                </span>
                                <span className="text-blue-500 ml-1">
                                  • {formatDate(parentTx.transaction_date)}
                                  {parentTx.description && ` • ${parentTx.description}`}
                                </span>
                              </div>
                            ))}
                          </div>
